...
def get_docker_image_id(image):
    ...
    MANIFEST = check_output(f'''curl -s -H "Authorization: Bearer {TOKEN}" \
    -H "Accept: application/vnd.oci.image.index.v1+json" \
    -H "Accept: application/vnd.docker.distribution.manifest.list.v2+json" \
    -H "Accept: application/vnd.docker.distribution.manifest.v2+json" \
    "https://registry-1.docker.io/v2/{image_name}/manifests/{tag}" ''', verbose=False)
    try:
        size = sum([int(x) for x in check_output(f"echo '{MANIFEST}' | jq '.layers[].size' ", verbose=False).split('\n')])
        size = round(size  / 1024 / 1024, 4)
    except:
        try:
            size = check_output(f"echo '{MANIFEST}' | jq '.manifests[0].size' ", verbose=False)
        except:
            size = -1
    # print(MANIFEST, )
    ret = check_output(f"echo '{MANIFEST}' | jq -r '.config.digest' ", verbose=False)
    if not ret or ret == 'null':
        ret = check_output(f"echo '{MANIFEST}' | jq -r '.manifests[0].digest' ", verbose=False)
    return ret, \
            check_output(f"echo '{MANIFEST}' | jq -r '.errors[0].message' ", verbose=False), \
            size
...
def check_identical(image, sensetime_image):
    a, err, size = get_docker_image_id(image)
...
if __name__ == '__main__':
    ...
    upload_once('opendilab/ding:nightly')